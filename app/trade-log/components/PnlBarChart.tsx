...
export default function PnlBarChart({ trades }: { trades: Trade[] }) {
  ...
  useEffect(() => {
    ...
    const data = closed.map((t) => t.realizedPnl);
    const labels = closed.map((t, i) => `#${i + 1} ${t.ticker}`);
    if (chartRef.current) chartRef.current.destroy();
    chartRef.current = new Chart(canvasRef.current, {
      type: "bar",
      data: { labels, datasets: [{ data, backgroundColor: data.map((v) => v >= 0 ? "#d1fae5" : "#fee2e2"), borderColor: data.map((v) => v >= 0 ? "#10b981" : "#ef4444"), borderWidth: 1.5, borderRadius: 4 }] },
      options: {
        ...
        scales: { x: { ticks: { color: "#d1d5db", font: { size: 9 }, maxTicksLimit: 10 }, grid: { display: false }, border: { color: "#e5e7eb" } }, y: { ticks: { color: "#9ca3af", font: { size: 9 }, callback: (v) => v != null ? `₹${Number(v).toLocaleString("en-IN")}` : "" }, grid: { color: "#f3f4f6" }, border: { color: "#e5e7eb" } } },
      },
    });
