    ...
    chartRef.current = new Chart(canvasRef.current, {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: data.map((v) =>
            v >= 2 ? "#d1fae5" : v >= 1 ? "#a7f3d0" : v >= 0 ? "#fef9c3" : "#fee2e2"
          ),
          borderColor: data.map((v) =>
            v >= 2 ? "#10b981" : v >= 1 ? "#34d399" : v >= 0 ? "#f59e0b" : "#ef4444"
          ),
          borderWidth: 1.5,
          borderRadius: 4,
        }],
      ...
      options: {
        ...
        scales: {
          ...
          y: {
            ticks: { color: "#9ca3af", font: { size: 9 }, callback: (val) => val != null ? `${Number(val).toFixed(1)}R` : "" },
